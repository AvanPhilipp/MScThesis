...
//	unsigned char arr_out[in_img.rows * in_img.cols];

	static xf::Mat<TYPE,HEIGHT,WIDTH,NPC1> imgStreamOutput(in_img.rows,in_img.cols);

	imgInput.copyTo(in_img.data);
	xf::xfMat2Array(imgInput,arrayInput);
	more_adaptive_thresh_accel(arrayInput,arrayOutput,2,imgInput.cols,imgInput.rows);
	xf::Array2xfMat(arrayOutput,imgOutput);
//	imgOutput.copyTo(arr_out);
	xf::imwrite("HLS_array_img.jpg", imgOutput);

	imgInput.copyTo(in_img.data);
	more_adaptive_thresh(imgInput,imgStreamOutput,2);
